...
exports.listen = function(port, collections, events, options) {
    ...
    compiledProtoJS = "(function() { window.Omni.CollectionProtos = {}; window.Omni.ModelProtos = {} })();";

    var compileProto = function(proto, type, modelName) {
        var protoIndex = compiledProtos.push(proto) - 1;
        compiledProtoJS += "(function() {";
        compiledProtoJS += "window.Omni." + type + "Protos." + proto.name + " = Backbone." + type + ".extend({";
        if (type == "Collection") {
            compiledProtoJS += "model: window.Omni.ModelProtos." + modelName + ",";
        }
        for (var y in proto) {
    ...
    for (var x in collections) {
        ...
        compileProto(collection.model.prototype, "Model");

        // Now compile the collection prototype
        compileProto(proto, "Collection", collection.model.prototype.name);
    }
