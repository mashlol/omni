        ...
        for (var collectionName in data) {
            ...
            (function(collectionName, modelName) {
                ...
                Omni.connection.on("change:" + modelName, function(data) {
                    var model = window.Collections[collectionName].find(function(model) { if (model.oid == data.oid) return true; });
                    delete data.oid;
                    model.set(data, {fromServer: true});
                });

                Omni.connection.on("oid:" + modelName, function(data) {
                    window.Collections[collectionName].find(function(model) { if (model.oid == data.clientOID) return true; }).oid = data.oid;
                });
